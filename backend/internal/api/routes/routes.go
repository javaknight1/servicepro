...
func Setup(router *gin.Engine, db *gorm.DB, redis *redis.Client, email emailclient.Client, storage storageclient.Client, cfg *config.Config) {
	...
	// Build RouteConfig
	routeCfg := NewRouteConfig(db, redis, email, storage, cfg)
...
func NewRouteConfig(db *gorm.DB, redis *redis.Client, email emailclient.Client, storage storageclient.Client, cfg *config.Config) *RouteConfig {
	...
	return &RouteConfig{
		Config:     cfg,
		Clients:    &Clients{DB: db, Redis: redis, Email: email, Storage: storage},
		Repos:      repos,
		Middleware: mw,
		Services:   svc,
		Handlers:   handlers,
