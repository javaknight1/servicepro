...
// NewClient creates a new tracing client based on configuration.
//
// Provider selection order:
// 1. If cfg.Server.Env == "development" -> Mock
// 2. If cfg.Sentry.DSN != "" -> Sentry
// 3. If cfg.OpenTelemetry.Enabled -> OpenTelemetry
// 4. Fallback -> Mock
//
// Note: Providers must be registered by importing their packages with blank imports:
//
//	import _ "github.com/javaknight1/servicepro/backend/pkg/clients/tracing/mock"
//	import _ "github.com/javaknight1/servicepro/backend/pkg/clients/tracing/sentry"
//	import _ "github.com/javaknight1/servicepro/backend/pkg/clients/tracing/otel"
func NewClient(ctx context.Context, cfg *config.Config) (Client, error) {
	...
	logging.Info(ctx, "[TRACING] Using provider", map[string]any{"provider": provider})
	...
	if !ok {
		...
		return nil, fmt.Errorf("tracing provider %q not registered; registered providers: %v. Import the provider package with a blank import", provider, registered)
	}
...
func detectProvider(cfg *config.Config) Provider {
	...
	// 3. Check for OpenTelemetry config
...
// MustNewClient creates a new tracing client or panics on error.
// This is useful for application initialization.
func MustNewClient(ctx context.Context, cfg *config.Config) Client {
	client, err := NewClient(ctx, cfg)
	if err != nil {
		panic(fmt.Sprintf("failed to create tracing client: %v", err))
	}
	return client
}
