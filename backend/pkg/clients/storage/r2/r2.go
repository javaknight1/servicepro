...
func init() {
	storage.RegisterProvider(storage.ProviderR2, func(ctx context.Context, cfg *config.Config) (storage.Client, error) {
		r2Cfg := &Config{
		...
		return NewClient(ctx, r2Cfg)
	})
}

// Config holds configuration for the Cloudflare R2 storage client
type Config struct {
	AccountID       string
	AccessKeyID     string
	SecretAccessKey string
	Bucket          string
	Endpoint        string // Optional: override the default R2 endpoint
	PublicURL       string // Optional: for public access URLs
}

// Client implements the storage.Client interface using Cloudflare R2
type Client struct {
	client    *s3.Client
...
func NewClient(ctx context.Context, cfg *Config) (*Client, error) {
	...
	if cfg.AccountID == "" && cfg.Endpoint == "" {
		return nil, fmt.Errorf("R2 account ID or endpoint is required")
	}
...
func (c *Client) uploadData(ctx context.Context, data []byte, input *storage.UploadInput) (*storage.UploadOutput, error) {
	...
	if c.config.PublicURL != "" {
		url = fmt.Sprintf("%s/%s", strings.TrimSuffix(c.config.PublicURL, "/"), key)
	}
...
func (c *Client) HealthCheck(ctx context.Context) error {
	...
	log.Printf("R2 health check passed for bucket %s", c.config.Bucket)
	return nil
}

// Close implements storage.Client
func (c *Client) Close() error {
	// S3-compatible client doesn't require explicit cleanup
	return nil
}
